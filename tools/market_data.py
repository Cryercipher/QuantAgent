...
class MarketDataManager:
    ...
    def get_tool(self):
        return FunctionTool.from_defaults(
            ...
            description=(
                "【行情与趋势分析器】专用于获取A股个股的**价格行为 (Price Action)** 数据。\n"
                "【核心数据】返回最新收盘价、近N日均价(MA)、近期价格走势列表。\n"
                "【强制调用场景】\n"
                "1. **判断买卖点**：当用户询问“现在能买吗”，你需要对比“最新价”与“近30日均价”的位置关系（是高位还是低位）。\n"
                "2. **评估风险**：通过观察返回的“近期走势”列表，判断股价是否剧烈波动。如果走势平稳，适合稳健型；如果大起大落，则风险高。\n"
                "3. **SOP 规定**：凡是涉及“价格”、“表现”、“走势”或“建议”，**必须**先调用此工具拿到客观价格，严禁仅凭理论分析。\n"
            )
        )